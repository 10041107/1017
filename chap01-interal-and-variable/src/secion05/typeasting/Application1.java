...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        (long)10 * (int)10 >> 둘다 long으로 변환해서 계산함., 타입캐스팅
        1-1. 작은 자료형에서 큰 자료형으로는 자동 형변환 된다.
        1-2. 정수는 실수로 자동 형변환 된다.
        1-3. 문자열은 int 형으로 자동 형변환 된다.
        1-4. 논리형은 형변환 규칙에서 제외된다.

        2. 강제 형변환(명시적 형변환): 형변환 cast연산자를 이용한 강제적으로 수행하는 형변환
    ...
    // 실수와 정수 연산
        ...
        System.out.println(chValue); //그냥 출력
        System.out.println((int)ch1); // int로 형변환
        System.out.println((char) charNumber); // char로 출력
        char ch2 = 65; // = 아스키코드: a
        ...
        //큰 것에서 작은것을 연산하면 에러
        // = int에 long를 넣으려 하면 불가능함
         // = 형변환 했을때 손실 나는 건 감수해야함
