...
public class Appliation2 {

    public static void main(String[] args) {
        /*
        문자열을 특정 구분자로 하여 분리한 문자열을 반환하는 기능을 한다.
        split(): 정규 표현식을 이용하여 문자열을 분리한다.
                 비정형화된 문자열을 분리할 때 좋지만(공백 문자열 값 포)
                 정규 표현식을 이용하기 때문에 속도가 느리다는 단점을 지닌다.
                 정규 표현식은 자바에서 다루지 않고 자바스크립트에서 다룰 예정이다.
         SringTokenizer : 문자열의 모든 문자들을 구분자로 하여 문자열을 분리한다.
                          정형화된 문자열 패턴을 분리하기 좋다. (공백 무시)
                          split()보다 속도면에서 빠르다.
        ...
        } // nextToken : 공백을 포함하지 않음

        String colorStr = "red*orange#blue/yello green";
//        String [] colorArray = colorStr.split("/#*"); //  불가능함
//
...
//        }
        ...
        String[] colors = colorStr.split("[*#/ ]"); // 기호를 삭제하려면 [] 반드시 필요
