...
public class Application01 {
    public static void main(String[] args) {
        ...
        1. 새로운 클래스를 작성할 시 기존에 작성한 클래스를 재사용할 수 있다
            ...
            1-2. 공통적으로 사용하는 코드가 부모클래스에서 존재하면 수정사항이 생길 시 부모클래스만 수정해도
            전체적으로 적용된다.(유지보수성 증가)
        2. 클래스 간의 계층 관계가 형성되며 다형성 문법적인 토대가 된다.

        하지만 상속으로 인한 단점도 존재한다
        1. 부모 클래스의 기능을 추가/변경할 시 자식클래스가 정상적으로 동작하는지에 대한 예측이 힘들다.
        상속 구조가 복잡해 질수록 그 영향에 대한 예측이 힘들며 이러한 단점이 유지보수성을 증가한단느 장범과는 반대로
        유지보수에 악영향을 미친다
        2. 또한 부모 클래스의 변경 또한 쉽지 않다. 자식 클래스에서 중요하게 사용하는 기능일 경우
        부모 클래스를 변경할 시 자식 클래스에 모두 영향을 줄 수 있다
        역시 유지보수에 악영향을 미친다
        3. 부모클래스에서는 의미있었던 기능이 자식 클래스에서는 무의미할 수 있다. (불필요한 기능이 추가됨)

        장점과 단점을 고려했을 때
        상속은 재사용이라는 장점만 바라보게 되면 오용의 가능성이 있기 때문에 유지보수에 좋지 않은 코드를 작성할 확률이 높다.
        상속으느 IS-A(중요) 관계로 구분이 되는 경우에만 사용해야 한다.
        ...
        역할이랑 동일한 동작을 수행하는 것을 정의한 것이며 개체 가능성을 의미한다.
        부모클래스를 추상화하는 경우에는 역할의 관점을 바라봐야 한다.
        ...
        Car test = fireCar; //자식에게 오버라이드 된 값을 그대로 따라감
        test.isRunning();
        test.soundHorn();
...
//        FireCar fireCar1 = racingCar; // fireCar자식과 racingCar자식은 별도의 개체. Car는 부모라 가능하지만 서로는 안됨
